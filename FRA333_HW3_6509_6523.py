...
def endEffectorJacobianHW3(q:list[float])->list[float]:
    ...
    for i in range(len(q)):
        ...
        Z_i = R[:, 2, i] # Rotation of each joint
        J[:3, i] = (np.cross(Z_i, p_e - P_i)) @ R_e # Add Linear Jacobian from row 1 to row 3 that reference {e}
        J[3:, i] = Z_i # Add Angular Jacobian from row 4 to row 6
